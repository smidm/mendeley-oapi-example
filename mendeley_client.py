...
class MendeleyClientConfig:
    ...
    def is_valid(self):
        if not hasattr(self,"client_id") or not hasattr(self, "client_secret") \
            or not hasattr(self,"redirect_url"):
            return False

        if self.client_id == "<change me>" or self.client_secret == "<change me>" \
            or self.redirect_url == "<change me>":
            return False
...
class MendeleyClient(object):
    ...
    def exchange_access_token(self, code, redirect_url="http://localhost"):
        """Generate an access_token from a request_token generated by
           get_auth_url and the verifier received from the server"""
        try :
            access_token = self.oauth_client.get_access_token(code, redirect_url)
            print access_token
    ...
    def interactive_auth(self, redirect_url):
        auth_url = self.get_auth_url(redirect_url)
        print 'Go to the following url to auth the token:\n%s' % (auth_url,)
        code = raw_input('Enter the code from the end of the redirected url (everything after code=): ')
        self.set_access_token(self.exchange_access_token(code, redirect_url))
...
def create_client(config_file="config.json", keys_file=None, account_name="test_account"):
    ...
    if not access_token:
        try:
            client.interactive_auth(config.redirect_url)
